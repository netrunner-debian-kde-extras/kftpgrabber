...
OtpGenerator::OtpGenerator(const QString &challenge, const QString &password)
  : m_algorithm(None)
{
  m_password = password;

  // Parse the string and set the alg (looks like this: otp-md5 498 gr4542)
  ...
  if (alg == "otp-md4" || alg == "s/key")
    ...
    m_algorithm = AlgoRMD160;
  else if (alg == "otp-sha1")
    m_algorithm = AlgoSHA1;

  // Extract the actual challenge
  m_seq = challenge.section(' ', 1, 1).toInt();
  m_seed = challenge.section(' ', 2, 2).trimmed().toLower();
}

void OtpGenerator::genDigest(char *msg, unsigned int len)
{
  switch (m_algorithm) {
    case AlgoMD4: genDigestMD(4, msg, len); break;
    case AlgoMD5: genDigestMD(5, msg, len); break;
    case AlgoRMD160: genDigestRS(0, msg, len); break;
    case AlgoSHA1: genDigestRS(1, msg, len); break;
    default: return;
  }
}

QString OtpGenerator::response()
{
  if (m_algorithm == None)
    return QString::null;
  ...
  strcpy(tmp, QString(m_seed + m_password).toAscii().data());
...
void OtpGenerator::genDigestMD(int type, char *msg, unsigned int len)
{
...
void OtpGenerator::genDigestRS(int type, char *msg, unsigned int len)
{
...
unsigned short OtpGenerator::extract(char *s, int start, int len)
{
  unsigned char cl;
  unsigned char cc;
  unsigned char cr;
...
unsigned char OtpGenerator::parity(unsigned char *msg)
{
...
unsigned char *OtpGenerator::sixWords(unsigned char *msg, char *response)
{
