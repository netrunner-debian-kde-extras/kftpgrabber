...
void ChecksumVerifierThread::checkSFV(const QString &sfvfile, const QString &fileToCheck)
{
  ...
  int done = 0;
  for (QList<QPair<QString, QString> >::iterator i = fileList.begin(); i != fileList.end(); ++i) {
    ...
    long l_fileCRC = getFileCRC(QString(fileDir + (*i).first).toAscii());
    QString fileCRC;
    fileCRC.sprintf("%08lX", l_fileCRC);
    fileCRC = fileCRC.toLower();
...
void ChecksumVerifier::customEvent(QEvent *e)
{
